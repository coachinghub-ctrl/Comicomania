-- ---------------------------------------------------------------------------
-- La misma puerta, con la firma ordenada.
--
-- En Postgres un parámetro con valor por defecto obliga a que todos los
-- siguientes también lo tengan. La primera versión dejaba la categoría y la
-- descripción en medio sin defecto, así que desde el cliente había que
-- mandarlas siempre aunque fueran opcionales de verdad.
--
-- Lo obligatorio delante, lo opcional detrás. Mismo comportamiento.
-- ---------------------------------------------------------------------------

drop function if exists public.inscribirme_en_concurso(
  uuid, uuid, text, text, text, integer, jsonb, text);
...
begin
  ...
  select id, status, registration_opens_at, registration_closes_at,
         submission_deadline, age_reference_date, name, rules_document_id
    into v_concurso
  ...
  /* El perfil completo no es burocracia: sin fecha de nacimiento no se puede
     ...
     el mínimo legal. */
  select id, profile_complete, birth_date into v_persona
  ...
  if p_categoria is not null then
    ...
    if v_categoria.max_age is not null and v_edad > v_categoria.max_age then
      raise exception 'La categoría % es hasta % años, y tú tienes %.',
        v_categoria.name, v_categoria.max_age, v_edad using errcode = 'P0001';
    end if;
  end if;
  ...
  /* Las bases vigentes DE ESTE concurso. La aceptación apunta a una versión
     concreta, porque "aceptó los términos" sin decir cuáles no sirve de nada
     el día que haya una discusión. */
  select v.id into v_version
    from public.legal_document_versions v
   where v.document_id = v_concurso.rules_document_id
     and v.status = 'EFFECTIVE'
   order by v.version desc
   limit 1;

  if v_version is null then
    raise exception 'Este concurso todavía no tiene bases publicadas, así que no hay nada que aceptar.'
      using errcode = 'P0001';
  end if;

  -- Las cuatro casillas, comprobadas aquí y no solo en el formulario: un
  ...
  /* El video entra como ENVIADO con los derechos DECLARADOS, no aclarados:
     ...
     nadie lo haya comprobado. Eso es la revisión. */
...
revoke all on function public.inscribirme_en_concurso(
  uuid, text, text, jsonb, uuid, text, integer, text) from public;
grant execute on function public.inscribirme_en_concurso(
  uuid, text, text, jsonb, uuid, text, integer, text) to authenticated;
